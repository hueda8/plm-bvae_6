...
def clean_sequence(seq: str, mode: str = "esm"):
    ...
    if mode == "strict":
        ...
        return "".join([c if (c in VALID_AA) else ("X" if c in {"U", "Z", "O"} else c) for c in raw])
    else:
        raise ValueError(f"Unknown clean mode: {mode}")

def split_chunks(seq: str, max_len: int) -> List[str]:
...
def load_model(encoder_model: str, tokenizer_model: str, amp_dtype: torch.dtype, device: str, lora_path: str = None, state_path: str = None):
    ...
    if device == "cuda" and amp_dtype in (torch.float16, torch.bfloat16):
        model = EsmModel.from_pretrained(encoder_model, torch_dtype=amp_dtype)
    else:
        model = EsmModel.from_pretrained(encoder_model)
    if state_path:
        sd = torch.load(state_path, map_location=device)
        missing, unexpected = model.load_state_dict(sd, strict=False)
        print(f"[FullFT] Loaded ESM2 state from {state_path}. Missing: {len(missing)} Unexpected: {len(unexpected)}")
    if lora_path:
        if not _HAS_PEFT:
...
def embed_single_chunk(seq: str, tokenizer, model, device: str, add_special_tokens: bool, amp_dtype: torch.dtype) -> np.ndarray:
    toks = tokenizer(seq, return_tensors="pt", add_special_tokens=add_special_tokens, padding=False, truncation=False)
...
def embed_sequence(seq: str, tokenizer, model, device: str, add_special_tokens: bool, emb_dim: int, amp_dtype: torch.dtype) -> np.ndarray:
    if len(seq) == 0: return np.zeros((0, emb_dim), dtype=np.float32)
    if len(seq) <= MAX_TOKENS:
        return embed_single_chunk(seq, tokenizer, model, device, add_special_tokens, amp_dtype)
    parts = split_chunks(seq, MAX_TOKENS)
    arrays = [embed_single_chunk(p, tokenizer, model, device, add_special_tokens, amp_dtype) for p in parts]
    arr = np.concatenate(arrays, axis=0)
    if arr.shape[0] != len(seq): raise RuntimeError(f"[LONG SEQ MISMATCH] seq_len={len(seq)} emb={arr.shape[0]}")
    return arr
...
def run(input_file: str,
    ...
    long_idx = [i for i, s in enumerate(sequences) if len(s) > MAX_TOKENS]
    ...
    with torch.inference_mode():
        for batch in tqdm(batches, desc="Embedding (normal)"):
            need = [i for i in batch if i not in existing]
            if not need: continue
            seqs = [sequences[i] for i in batch]
            toks = tokenizer(seqs, return_tensors="pt", add_special_tokens=add_special_tokens, padding=True, truncation=False)
            toks = {k: v.to(device) for k, v in toks.items()}
            with torch.autocast(device_type='cuda', dtype=amp_dtype, enabled=use_autocast):
    ...
    for idx in tqdm(long_idx, desc="Embedding (long)"):
        if idx in existing: continue
        arr = embed_sequence(sequences[idx], tokenizer, model, device, add_special_tokens, emb_dim, amp_dtype)
        diff_counter[arr.shape[0] - len(sequences[idx])] += 1
        np.save(os.path.join(out_dir, f"{idx}.npy"), arr)

    for idx in empty_idx:
        if idx in existing: continue
        np.save(os.path.join(out_dir, f"{idx}.npy"), np.zeros((0, emb_dim), dtype=np.float32))
        diff_counter[0] += 1
...
def main():
    ...
    cfg = {
        ...
        "encoder_model": hp.get("plm_encoder_model", "facebook/esm2_t36_3B_UR50D"),
        "tokenizer_model": hp.get("plm_tokenizer_model", hp.get("plm_encoder_model", "facebook/esm2_t36_3B_UR50D")),
        "add_special_tokens": hp.get("plm_add_special_tokens", True),
        "clean_mode": hp.get("plm_clean_mode", "esm"),
