...
MAX_TOKENS = 1022  # 安全側の上限

def clean_sequence(seq: str, mode: str = "ankh"):
...
def load_model(encoder_model: str, tokenizer_model: str, amp_dtype: torch.dtype, device: str, lora_path: str = None, state_path: str = None):
    ...
    if device == "cuda" and amp_dtype in (torch.float16, torch.bfloat16):
        model = T5EncoderModel.from_pretrained(encoder_model, dtype=amp_dtype)
    else:
        model = T5EncoderModel.from_pretrained(encoder_model)
    if state_path:
        sd = torch.load(state_path, map_location=device)
        missing, unexpected = model.load_state_dict(sd, strict=False)
        print(f"[FullFT] Loaded Ankh state from {state_path}. Missing: {len(missing)} Unexpected: {len(unexpected)}")
    if lora_path:
        if not _HAS_PEFT:
    ...
    emb_dim = int(getattr(model.config, "hidden_size", getattr(model.config, "d_model", 1024)))
...
def run(input_file: str,
    ...
    with torch.inference_mode():
        for batch in tqdm(batches, desc="Embedding (normal: Ankh)"):
    ...
    for idx in tqdm(long_idx, desc="Embedding (long: Ankh)"):
...
def main():
    ...
    cfg = {
        ...
        "encoder_model": hp.get("plm_encoder_model", "ElnaggarLab/ankh-large"),
        "tokenizer_model": hp.get("plm_tokenizer_model", hp.get("plm_encoder_model", "ElnaggarLab/ankh-large")),
        "add_special_tokens": hp.get("plm_add_special_tokens", True),
        "clean_mode": hp.get("plm_clean_mode", "ankh"),
